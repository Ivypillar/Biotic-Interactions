...
GloNAF_shp <- read_sf("../External_Data/GloNAF/glonaf_2024_nol/glonaf_2024_nol.shp")
...
linked_tbl <- glonaf_flora2 %>% left_join(GBIF_glonaf_taxon_wcvp_author, by = c("taxon_wcvp_id"="taxon_orig_id") ) 
linked_tbl2 <- linked_tbl %>% left_join(glonaf_region, by = c("region_id"="id") ) 
linked_data2_coll <- collect(linked_tbl2)
linked_tbl3 <- linked_data2_coll %>% left_join(GloNAF_shp, by ="OBJIDsic") 
...
full_df <- mutate(harmonised_comp_rearr,
                  infoFlags = case_when(
                    glonaf_flag == 1  ~ "GIFT & GloNAF",
                    is.na(glonaf_flag) ~ "NA")
)
...
for (ID in GBIF_GloNAF_uniq) {

print(ID)
test_species_tab <-linked_tbl3[linked_tbl3$GBIF_taxonID==ID,]
test_species_int <-glonaf_sub2[glonaf_sub2$new_source_taxonID==ID,]
