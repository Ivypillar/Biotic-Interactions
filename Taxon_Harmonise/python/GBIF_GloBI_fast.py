...
def getBestMatch(GBIF_result_list,Phylo_array):
    print()
...
#with open(GloBI) as G:
    ...
    target_header = ["\"GBIF_target_taxonID\"", "\"GBIF_target_genericName\"", "\"GBIF_target_specificEpithet\"", "\"GBIF_target_kingdom\"", "\"GBIF_target_phylum\"", "\"GBIF_target_class\"", "\"GBIF_target_order\"", "\"GBIF_target_family\""]

    header_list = source_header+ target_header + globi_header2_sep
    ...
    for l in csv.reader(G, quotechar='"', delimiter=',', quoting=csv.QUOTE_ALL, skipinitialspace=True):
        ...
        sourceTaxonId, sourceTaxonIds, sourceTaxonName, sourceTaxonRank, sourceTaxonPathNames, sourceTaxonPathIds, \
                ...
                targetPhysiologicalStateId,targetPhysiologicalStateName,targetSexId,targetSexName,decimalLatitude,decimalLongitude,\
                localityId,localityName,eventDate,argumentTypeId,referenceCitation,referenceDoi,referenceUrl,sourceCitation,\
                sourceNamespace,sourceArchiveURI,sourceDOI,sourceLastSeenAtUnixEpoch = l

        #sourceTaxonName_array = sourceTaxonName.split(" ")
        source_Phylo = [sourceTaxonKingdomName,sourceTaxonPhylumName,sourceTaxonClassName,sourceTaxonOrderName,sourceTaxonFamilyName]
        target_Phylo = [targetTaxonKingdomName,targetTaxonPhylumName,targetTaxonClassName,targetTaxonOrderName,targetTaxonFamilyName]
        #print(source_Phylo)
        #print(target_Phylo)
        gbif_source = process_taxon(sourceTaxonName,source_Phylo)
        gbif_target =process_taxon(targetTaxonName,target_Phylo)

        output_list = gbif_source + gbif_target + l
